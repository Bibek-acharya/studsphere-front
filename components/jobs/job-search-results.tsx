...
export default function JobSearchResults({ jobs, searchParams }: JobSearchResultsProps) {
  const [keywords, setKeywords] = useState(searchParams.keywords)
  const [selectedLocations, setSelectedLocations] = useState(searchParams.location ? [searchParams.location] : [])
  const [selectedType, setSelectedType] = useState(searchParams.jobType ? [searchParams.jobType] : [])
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      // Keywords filter
      if (keywords && !job.title.toLowerCase().includes(keywords.toLowerCase())) {
        return false
      }
  ...
  }, [jobs, keywords, selectedLocations, experienceLevel, selectedType])
  ...
  const sortedJobs = useMemo(() => {
    ...
    if (sortBy === "newest") {
      sorted.sort((a, b) => {
        const aDays = Number.parseInt(a.jobExpire.split(" ")[0]) || 0
        const bDays = Number.parseInt(b.jobExpire.split(" ")[0]) || 0
        return bDays - aDays
      })
    }
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-lg p-4 sm:p-6 space-y-6">
            ...
            <div className="border-b pb-4">
              <h3 className="font-semibold text-gray-900 mb-3">Search Keywords</h3>
              <input
                type="text"
                value={keywords}
            ...
            <div className="border-b pb-4">
              ...
              <div className="relative">
                ...
                >
                  <span>
                    {selectedLocations.length > 0 ? `${selectedLocations.length} selected` : "Select Location"}
                  </span>
          ...
          <div className="mb-6 flex items-center justify-between">
            ...
            <button className="flex items-center gap-2 px-3 py-1 bg-gray-100 text-gray-700 rounded-lg text-sm hover:bg-gray-200 transition-colors">
              ...
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V17a2 2 0 01-2 2h-1C9.716 19 3 12.284 3 4V3z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 sm:items-start">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between mb-2">
                          ...
                          <Heart className="w-5 h-5 text-gray-400 hover:text-red-600 transition-colors shrink-0" />
                        </div>
                        ...
                        <p className="text-sm text-gray-600 mb-2 line-clamp-2">
                          {job.jobHighlights[0] || job.description}
                        </p>
