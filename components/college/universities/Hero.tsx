...
const Hero: React.FC<HeroProps> = ({ onSearch }) => {
  ...
  const handleSearch = () => {
    if (query.trim()) {
      router.push(`/colleges/search?q=${encodeURIComponent(query)}`);
    }
    onSearch(query);
  };
  ...
  return (
    <div className="w-full bg-white py-16 px-4 text-center">
      ...
      <p className="text-gray-500 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
        Stop searching blindly. Update your profile and let our algorithm match
        you with the opportunities you deserve.
      </p>
