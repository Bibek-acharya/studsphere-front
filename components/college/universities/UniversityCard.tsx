  ...
  return (
    <div className="bg-white rounded-4xl p-6 shadow-sm border border-gray-100 flex flex-col gap-6 hover:shadow-md transition-all">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {university.popularPrograms.map((program, idx) => (
            <span
              key={idx}
              className="bg-slate-50 text-slate-600 text-sm px-4 py-1.5 rounded-full border border-slate-100"
            >
              {program}
            </span>
          ))}
          <span className="text-slate-400 text-sm font-medium px-4 py-1.5">
            +More
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mt-auto">
        <Link
          href={`/colleges/universities/${university.id}`}
          className="flex-1"
        >
          <button className="w-full border border-gray-200 hover:bg-gray-50 text-slate-700 font-semibold py-3 rounded-xl transition-all">
            Details
          </button>
        </Link>
        ...
        <button
          ...
          className={`w-12 h-12 flex items-center justify-center border border-gray-200 rounded-xl transition-all ${
            isFavorite ? "bg-red-50 border-red-100" : "hover:bg-gray-50"
          }`}
        >
          <Heart
            size={20}
            className={`${
              isFavorite ? "text-red-500 fill-red-500" : "text-slate-400"
            }`}
          />
        </button>
      </div>
