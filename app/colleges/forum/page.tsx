...
const App: React.FC = () => {
  ...
  const addToast = useCallback((message: string, type: unknown = "success") => {
    const id = Math.random().toString(36).substring(2, 9);
    setToasts((prev) => [...prev, { id, message, type: type as ToastType }]);
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-bgLight">
      ...
      <div className="container mx-auto px-4 lg:px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <SidebarLeft activeNav={activeNav} setActiveNav={setActiveNav} />

        <main className="col-span-1 lg:col-span-6 space-y-6">
          <Feed openPostModal={openPostModal} addToast={addToast} />
        </main>
